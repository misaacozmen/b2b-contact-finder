...
class LifecycleSafetyTests(unittest.TestCase):
    ...
    def test_resume_recovery_resets_free_and_marks_paid_unknown(self):
    ...
    def _test_resume_recovery(self, path: Path):
        ...
        self.assertEqual(checkpoint.recover_interrupted_items(run_id), {"free_reset": 1, "paid_unknown": 1})
        items = checkpoint.load_run_items(run_id)
        self.assertEqual([item["free_state"] for item in items], ["PENDING", "DONE"])
        self.assertEqual(items[1]["paid_state"], "UNKNOWN")

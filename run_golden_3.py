"""Run the independent 15-company Golden 3 holdout after manual validation."""
...
GOLDEN_DIR = Path(__file__).resolve().parent / "outputs" / "golden_3_20260715"
GOLDEN_INPUT = GOLDEN_DIR / "golden_3_pipeline_input_15.xlsx"
GOLDEN_EXPECTED = GOLDEN_DIR / "golden_3_manual_validation_15.xlsx"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the independent Golden 3 holdout")
...
def main_cli() -> None:
    ...
    if issues:
        print("Golden 3 manuel dogrulamasi tamamlanmamis; API harcanmadan kosu durduruldu.")
    ...
    output_dir = config.OUTPUT_DIR / f"golden3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    print(f"Golden 3 cikti klasoru: {output_dir}")
    print(main.run(GOLDEN_INPUT, output_dir, selected or None))
    _print_comparison("Golden 3 - tum bulunan adaylar", output_dir / "contacts.xlsx", selected)
    _print_comparison(
        "Golden 3 - otomatik kullanima uygun dogrulanmis sonuclar",

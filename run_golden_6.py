"""Run the 20-company Golden 6 blind benchmark after manual validation."""
...
from validate_golden_xlsx import FIELDS, evaluate, evaluate_stages, readiness_issues


GOLDEN_DIR = Path(__file__).resolve().parent / "outputs" / "golden_6_20260718"
GOLDEN_INPUT = GOLDEN_DIR / "golden_6_discovery_blind_input_20.xlsx"
GOLDEN_EXPECTED = GOLDEN_DIR / "golden_6_manual_validation_20_ready.xlsx"
...
def _print_comparison(title: str, actual: Path, selected: set[str]) -> None:
    ...
    for field in FIELDS:
        ...
        print(
            f"{field}: TP {values['tp']}, FP {values['fp']}, "
            f"FN {values['fn']}, precision %{precision:.1f}"
        )
    print(f"tam firma eslesmesi: {len(complete)}/{len(selected) if selected else 20}")


def _print_stage_metrics(output_dir: Path) -> None:
    candidates = output_dir / "website_candidates.xlsx"
    contacts = output_dir / "contacts.xlsx"
    if not candidates.exists() or not contacts.exists():
        return
    stages = evaluate_stages(GOLDEN_EXPECTED, contacts, candidates)
    print("\nGolden 6 - asama metrikleri")
    print("=" * 24)
    for key in (
        ...
        "abstention_rate",
        "email_recall_given_correct_site",
        "phone_recall_given_correct_site",
    ):
        print(f"{key}: %{stages[key] * 100:.1f}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Golden 6 blind benchmark")
...
def main_cli() -> None:
    ...
    if issues:
        print("Golden 6 manuel dogrulamasi tamamlanmamis; API harcanmadan kosu durduruldu.")
    ...
    if args.hunter_budget is not None:
        config.HUNTER_REQUEST_HARD_CAP = config.HUNTER_REQUEST_BUDGET = max(0, args.hunter_budget)
    if args.brandfetch_budget is not None:
        config.BRANDFETCH_REQUEST_HARD_CAP = config.BRANDFETCH_REQUEST_BUDGET = max(0, args.brandfetch_budget)
    ...
    output_dir = config.OUTPUT_DIR / f"golden6_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    print(f"Golden 6 cikti klasoru: {output_dir}")
    print(main.run(GOLDEN_INPUT, output_dir, selected or None))
    _print_comparison("Golden 6 - tum bulunan adaylar", output_dir / "contacts.xlsx", selected)
    _print_comparison(
        "Golden 6 - otomatik kullanima uygun dogrulanmis sonuclar",
        output_dir / "verified_contacts.xlsx",
        selected,
    )
    _print_stage_metrics(output_dir)

...
RUNTIME_ROOT = (
    Path(os.environ["B2B_RUNTIME_ROOT"]).expanduser().resolve()
    if os.getenv("B2B_RUNTIME_ROOT", "").strip()
    else BASE_DIR
)
...
REPLAY_SNAPSHOT_MAX_UNCOMPRESSED_BYTES = int(
    os.getenv("REPLAY_SNAPSHOT_MAX_UNCOMPRESSED_BYTES", str(64 * 1024 * 1024))
)
REPLAY_MANIFEST_INPUT = None
REPLAY_SNAPSHOT_CHECKPOINT_INTERVAL = max(
    1, int(os.getenv("REPLAY_SNAPSHOT_CHECKPOINT_INTERVAL", "50"))
)
ALL_RESULTS_FILE = OUTPUT_DIR / "all_results.xlsx"
...
ZUCHEX_EVENT_ID = os.getenv("ZUCHEX_EVENT_ID", "").strip()
ZUCHEX_FILTER_ID = os.getenv("ZUCHEX_FILTER_ID", "").strip()
ZUCHEX_FILTER_VALUE_ID = os.getenv("ZUCHEX_FILTER_VALUE_ID", "").strip()
ALLOW_PAID = os.getenv("ALLOW_PAID", "0") == "1"
...
CRAWLER_SOURCE_PROFILE_HTTP_LIMIT = max(0, int(os.getenv("CRAWLER_SOURCE_PROFILE_HTTP_LIMIT", "2")))
CRAWLER_IDENTITY_HTTP_LIMIT = max(0, int(os.getenv("CRAWLER_IDENTITY_HTTP_LIMIT", "9")))
CRAWLER_CONTACT_HTTP_LIMIT = max(0, int(os.getenv("CRAWLER_CONTACT_HTTP_LIMIT", "5")))
CRAWLER_RECOVERY_HTTP_LIMIT = max(0, int(os.getenv("CRAWLER_RECOVERY_HTTP_LIMIT", "3")))
SOURCE_PREFLIGHT_HOST_HTTP_LIMIT = max(0, int(os.getenv("SOURCE_PREFLIGHT_HOST_HTTP_LIMIT", "2")))
DDGS_MAX_PHYSICAL_ATTEMPTS = 3
DDGS_MAX_EMPTY_ATTEMPTS = 2
MAX_DISCOVERY_QUERIES_PER_COMPANY = 6
MAX_TARGETED_QUERIES_PER_COMPANY = 4
INITIAL_IDENTITY_CANDIDATE_LIMIT = 3
ABSOLUTE_IDENTITY_CANDIDATE_LIMIT = 6
...
PAID_ENABLED = os.getenv("PAID_ENABLED", "1") != "0"
...
SOURCE_PROFILE_MAX_TRANSIENT_RETRIES = max(1, int(os.getenv("SOURCE_PROFILE_MAX_TRANSIENT_RETRIES", "2")))
SOURCE_PROBE_LEASE_SEC = max(1, int(os.getenv("SOURCE_PROBE_LEASE_SEC", "30")))
